...
- **Iteration loop is the workflow**: `python scripts/iterate.py` runs 12 criteria (ADR compliance, tests, docs, deploy artifacts, no-secrets) and suggests the next open item; `--full` runs the whole suite, `--log "msg"` records the iteration. Add new criteria in `scripts/criteria.py`.

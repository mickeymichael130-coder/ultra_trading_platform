...
| 16 | 2026-08-02 | **Secret-scanning in CI + C12 criterion**: added .gitleaks.toml (default rules + allowlist for venv/ & __pycache__/ generator artifacts) and a `secrets` job to ci.yml (gitleaks:v8.24.0 over a fetch-depth:0 clone, exit-0 verified on a fresh clone); new C12 checks tracked files for key/token assignments + key-material and bearer-token markers (placeholder-aware). Hygiene cleanup: the repo was tracking a full `venv/` (14,993 files, incl. the pydeck source-map gitleaks flagged) + `logs/bot.log` — both untracked (git rm --cached, files kept on disk) and `logs/` added to .gitignore; tracked file count dropped 15,091 -> 97. All 12 criteria PASS | |
| 17 | 2026-08-02 | **History rewrite + strict gitleaks**: force-pushed a rewritten `main` dropping the stale pre-rewrite commits (`d9addffa` intial commit, `cfe62efc` gitignore — the venv/__pycache__/logs/data artifacts + original token lived there). Rewrite used `git replace --graft` + `git-filter-repo` (orphan at `first commit` + strip `venv`, `logs`, `data`, `**/__pycache__` from every tree) after verifying the naive `--commit-callback` orphan silently dropped files added in cut commits (reproduced on a throwaway repo). History is now 7 linear commits; every commit's tree verified byte-identical to the original minus junk paths (0 files lost); fresh-clone gitleaks now exits 0 with the allowlist REMOVED (strict). `.gitleaks.toml` now runs pure default rules. All 12 criteria PASS | |
| 18 | 2026-08-03 | **Binance reconnect + stale-stream watchdog**: fixed a real bug seen in live paper runs ("Stale tick data: 30732s since last tick" — the stream dropped and never recovered). Root causes: `_receive_loop` kept the dead socket on `ConnectionClosed` so `_ensure_streams()` early-returned forever (no reconnect); no watchdog for a half-open socket that stops delivering data without raising. Fixes in `src/broker/binance_client.py`: `_receive_loop` now clears `self.websocket`/`_current_stream_url` before reconnecting; client tracks `_last_tick_time` on trade/kline messages; `_heartbeat_loop` gained a stale watchdog using the new `stale_timeout` (default 90s) that tears down the dead socket and forces `_reconnect()` (guarded against racing an in-flight refresh/reconnect). 4 new offline tests (ConnectionClosed → socket replaced + exactly one reconnect; tick updates `_last_tick_time`; stale stream → forced reconnect; dead socket → reconnect). Suite grew 189 → 193 tests, all passing (153s); all 12 criteria PASS | |
| 19 | 2026-08-03 | **Professional terminal UI for the dashboard**: replaced the demo-style Streamlit UI with a dark, trading-terminal look (palette `#0F172A`/`#111827`/`#1E293B`/`#334155`, accent blue/green/amber/red). New `src/dashboard/theme.py` (centralized colors + global CSS), `db.py` (defensive SQLite helpers), `components.py` (KPI cards, badges, dark plotly styling). `app.py` rebuilt with a consistent 12-page sidebar (Dashboard, Markets, Trading Engine, Portfolio, Trade History, Analytics, Backtesting, Strategy Lab, Risk Center, Signals, Settings, Logs). Highlights: Dashboard = 6 KPI cards + equity curve + live candlestick + strategy/risk/allocation + recent trades/logs/notifications; Trading Engine = connection/broker/mode/signal KPIs + open-position panel + CPU/RAM gauges; Portfolio = equity/margin/exposure/drawdown; Trade History + Analytics = filters, monthly returns, equity curve; Backtesting page runs the real `BacktestEngine` on DB candles (equity curve + stats + trade list); Strategy Lab = per-strategy stat cards; Risk Center = drawdown/daily-loss/emergency-stop; Signals/Logs keep AI-note expanders + severity search. All 5 dashboard AppTests still pass (nav radio label `🔔 Signals` + first-sidebar text-input DB path preserved). Suite: 193 tests | |
| 20 | 2026-08-03 | **Dashboard performance pass + C13 gate**: warm page reruns cut from 2–5.4s to 1.1–2.3s by emitting each KPI row as a single HTML grid (`kpi_row`) and moving every chart into cached plotly builders in a new `src/dashboard/charts.py` (`st.cache_data` 5s TTL: equity, live market, market+volume, allocation, analytics trio, risk/signal distributions). Cold-start imports (~26–45s on this host) remain an environment cost, not code — documented as a known constraint; warm interactions no longer queue up. New static **C13 perf gate**: `src/dashboard` must not use deprecated `use_container_width` (must use `width=\"stretch\"`) and charts.py must cache via `st.cache_data`. Loop grown to 13 criteria (11 executed without `--full`); all 13 PASS (C5/C6 SKIP without `--full`); 193 tests green | |
| 21 | 2026-08-03 | **Phase docs 03–16 written** (docs-first completion): every remaining phase spec filled from the `docs/phases/` template and grounded in the actual shipped code — `Phase_03_Market_Data` (CandleBuilder, session/weekend gate), `Phase_04_Indicators` (IndicatorEngine/MarketSnapshot), `Phase_05_Strategies` (EMACrossoverStrategy + Signal contract), `Phase_06_Risk_Engine` (RiskManager 10-check gate, kill switch), `Phase_07_Execution` (paper/live fills, core Order), `Phase_08_Position_Manager` (SL/TP/trailing/time exits + ExitReason), `Phase_09_Portfolio` (future/multi-broker contract), `Phase_10_Database` (SQLite store + WAL), `Phase_11_Backtesting` (BacktestEngine, shared live path), `Phase_12_Optimization` (grid search over BacktestEngine), `Phase_13_Dashboard` (12-page terminal UI + C13 perf), `Phase_14_Monitoring` (health probes), `Phase_15_Live_Trading` (token-gated, contract + blockers), `Phase_16_AI` (MarketAdvisor/SignalEnhancer AI notes). Phases index in `docs/phases/README.md` updated (03–14, 16 → built; 15 token-gated; 09 future). Docs-only iteration: no code changed, suite/criteria unaffected | |

## Open items for next iteration

- **Iteration loop is the workflow**: `python scripts/iterate.py` runs 13 criteria (ADR compliance, tests, docs, deploy artifacts, no-secrets, dashboard perf gate) and suggests the next open item; `--full` runs the whole suite, `--log "msg"` records the iteration. Add new criteria in `scripts/criteria.py`.
- **Fill out remaining phase docs** (Phase 01 + any future phase — 03–16 now written): from the `docs/phases/` template as each area is touched; index in `docs/phases/README.md` is now accurate.
